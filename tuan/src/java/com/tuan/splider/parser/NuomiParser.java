 ...
 *
 */
public class NuomiParser  implements ArticleParser {
	...
	public static void parserContent(String content, Article article) {
		int index = content.indexOf("class=\"layout product-main\"");
		if (index > 0) {
			content = content.substring(index + 27);
...
//				 System.out.println(title);
		...
		index = content.indexOf("class=\"cur-price\"");
		if (index > 0) {
...
//						 System.out.println(saveMoney);
					...
					index = content.indexOf("class=\"product-pic\"");
					if (index >= 0) {
...
//						 System.out.println(url);
	...
	public static void main(String[] args)throws Exception {
		NuomiParser p = new NuomiParser();
			p.parse("http://www.nuomi.com/", 1, 5);
	}
}